...
int main(int argc, char const *argv[]){
    ...
    for(int t=1;t<=test_times;++t) {
        ...
        if(system("diff .ans.txt .out.txt -Z")) {
            printf("Wrong Answer on test #%d\n",t);
            printf("Press Ctrl-C to exit, Press Enter to continue\n");
            printf("to see input  :> more .in.txt\n");
            printf("to see differ :> diff .ans.txt .out.txt\n");
            getchar();
        }
    }
    return 0;
